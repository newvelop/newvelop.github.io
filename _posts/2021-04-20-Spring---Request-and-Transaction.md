...
- readOnly : 읽기전용모드로 설정하는 속성으로, 강제로 flush 하지 않는 한 세션을 flush 하지 않는다. 따라서 트랜잭션이 커밋되더라도 영속성 컨텍스트의 변경사항이 flush 되지 않으며, 영속성에 담긴 엔티티의 변경 비교를 하지 않아 성능향상을 할 수 있다. 쓰기작업을 시도할 경우 예외가 발생한다.

- 롤백 : 트랜잭션에 예외가 발생했을시 어떻게 처리할까 지정한 속성으로써, checked exception은 롤백하지 않으며 runtime exception은 롤백을 하는게 기본 속성이다.
  - rollbackFor : checked exception은 롤백하지 않는게 기본속성이지만 강제로 특정예외 발생시 롤백을 시키고 싶을 경우 해당 예외를 설정하는 부분이다.
  - noRollbackFor : 특정 runtime exception이 일어나도 롤백하지 않고 싶을 경우 설정하는 부분이다.
